...
export const handleInputSubmitLogic = async ({
  ...
  if ((e.key === "Enter" || e.type === "click") && !isLoading) {
    ...
    try {
      ...
      let modelResponse = "";
      let lastMessageId = null; // Track the last AI message ID

      const updateCallback = (chunkText) => {
        modelResponse += chunkText;

        if (lastMessageId) {
          // Update the existing AI message
          dispatch(updateMessage({ id: lastMessageId, text: modelResponse }));
        } else {
          // Create a new AI message and store its ID
          lastMessageId = uuidv4();
          dispatch(addMessage({ id: lastMessageId, role: "model", text: modelResponse }));
        }
      };


      // Call the chat API with enhanced prompt and streaming callback
