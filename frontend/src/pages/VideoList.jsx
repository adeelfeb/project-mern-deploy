...
const VideoList = () => {
    ...
    const fetchVideos = useCallback(async (pageToFetch = currentPage, showLoader = true) => {
        ...
        try {
            ...
            const paginatedData = await adminService.getAllVideos(pageToFetch, limit);
            // console.log("All videos data fetched: ",paginatedData)
    ...
    const handleBulkDelete = async () => {
        ...
        if (window.confirm(`Are you sure you want to permanently delete ${selectedVideos.length} selected video(s)?`)) {
            setDeleting(true);
            try {
                await adminService.deleteVideos(selectedVideos);
